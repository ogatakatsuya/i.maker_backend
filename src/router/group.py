...
from src.schema.group import CreateGroupRequest, RegisterScoreRequest, CreateGroupResponse, RegisterScoreResponse, GetGroupsResponse, GetGroupsByQuizSetIdResponse
...
async def get_groups_by_quiz_set_id(quiz_set_id: int, db: AsyncSession = Depends(get_db)):
...
async def register_group(group_info: CreateGroupRequest, quiz_set_id: int, db: AsyncSession = Depends(get_db)):
    try:
...
async def register_score(score: RegisterScoreRequest, group_id: int, db: AsyncSession = Depends(get_db)):
